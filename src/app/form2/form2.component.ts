...
export class Form2Component implements OnInit {
  ...
  createOrderFormGroup(item):FormGroup{
   ...
   UpdateOrder(type){
    this.http.post(environment.apiUrl + '/update_quotes',{quote_id:this.quoteid,user_id:localStorage.getItem('userid'),order_status:type,expiry_date:this.fabricForm.get('delivery_date').value,order_id:this.id }).subscribe(response => {
      if(response['response']=='success'){
