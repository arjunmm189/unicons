...
@Component({
  selector: 'app-supplierreadyaccsandready',
  templateUrl: './supplierreadyaccsandready.component.html',
  styleUrls: ['./supplierreadyaccsandready.component.css']
})
export class SupplierreadyaccsandreadyComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private route: ActivatedRoute,private http:HttpClient,private router:Router,private toastr:ToastrService) {
    this.route.paramMap.subscribe(params => {
      ...
      if(this.id=='Accesories'||this.id=='Ready mades'){
  ...
  this.getProducrbyID();
    ...
    let QWERArray=['Accessories','Ready mades']
   }
  ...
  getProducrbyID(){
    this.http.post(environment.apiUrl + '/prod_list_byid',{prod_id:this.id,user_id:localStorage.getItem('userid')}).subscribe(response => {
      if(response['response']=='success'){
        ...
        this.profileForm.get('item_code').setValue(this.ProdList[0].item_code);
        ...
        this.profileForm.get('quantity').setValue(this.ProdList[0].quantity);
        this.profileForm.get('size').setValue(this.ProdList[0].size);
        this.profileForm.get('prod_type').setValue(this.ProdList[0].prod_type)
...
CustomerSave(){
  ...
  this.http.post(environment.apiUrl + '/prod_dtls_save',uploadData).subscribe(response => {
    if(response['response']=='success'){
      if(this.id=='Accesories'||this.id=='Ready mades'){
        this.router.navigate(['/machineandreadylist',this.id])
        Swal.fire({
          position: 'top-end',
          icon: 'success',
        ...
        this.router.navigate(['/machineandreadylist',this.type])
      }
